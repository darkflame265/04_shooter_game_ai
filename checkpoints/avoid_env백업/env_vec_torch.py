...
class VecShooterEnvTorch:
    ...
    def get_rng_state(self) -> Optional[torch.Tensor]:
        """
        Returns the internal torch.Generator state for this env (device-specific).
        Safe to store in checkpoint.
        """
        try:
    ...
    def set_rng_state(self, state: torch.Tensor) -> None:
        """
        Restores the internal torch.Generator state for this env.
        """
        if state is None:
            return
        try:
